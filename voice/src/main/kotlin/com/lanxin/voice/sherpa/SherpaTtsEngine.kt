 ...
 * - stub:// 路径：返回 Stub 状态
 * - 合成结果目前仅返回 spokenChars；PCM 播放层后续接 AudioTrack
 */
class SherpaTtsEngine(
    ...
    override suspend fun speak(text: String): TtsResult = withContext(Dispatchers.IO) {
        ...
        Log.i(TAG, "speak ok chars=${t.length} pcm=${pcm.size}B rate=${audio.sampleRateHz} ~${durationMs}ms")
        TtsResult(
            ok = true,
            spokenChars = t.length,
            detail = "native:${bridge.currentMode()}:${durationMs}ms"
        )
    }

    override suspend fun stop() {
        // OfflineTts 无 cancel；播放层 stop 后续接
    }
